...
RETURNS bytea
AS 'MODULE_PATHNAME', 'combinable_array_agg_serialize'
LANGUAGE C STRICT IMMUTABLE PARALLEL SAFE;

CREATE FUNCTION array_agg_deserialize(bytea, internal)
RETURNS internal
AS 'MODULE_PATHNAME', 'combinable_array_agg_deserialize'
LANGUAGE C STRICT IMMUTABLE PARALLEL SAFE;
...
CREATE FUNCTION combinable_string_agg_serialize(internal)
RETURNS bytea
AS 'MODULE_PATHNAME', 'combinable_string_agg_serialize'
LANGUAGE C IMMUTABLE PARALLEL SAFE;

CREATE FUNCTION combinable_string_agg_deserialize(bytea, internal)
RETURNS internal
AS 'MODULE_PATHNAME', 'combinable_string_agg_deserialize'
LANGUAGE C IMMUTABLE PARALLEL SAFE;
...
CREATE AGGREGATE combinable_string_agg(text, text) (
  ...
  serialfunc = combinable_string_agg_serialize,
  deserialfunc = combinable_string_agg_deserialize,
  parallel = safe
);
...
CREATE AGGREGATE partial_combine_string_agg(internal) (
  sfunc = combinable_string_agg_combine,
  finalfunc  = combinable_string_agg_serialize,
  stype = internal,
  combinefunc = combinable_string_agg_combine,
  serialfunc  = combinable_string_agg_serialize,
  deserialfunc  = combinable_string_agg_deserialize,
