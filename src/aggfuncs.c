...
/*
 * combinable_array_agg_serialize
 */
PG_FUNCTION_INFO_V1(combinable_array_agg_serialize);
Datum
combinable_array_agg_serialize(PG_FUNCTION_ARGS)
{
...
/*
 * combinable_array_agg_deserialize
 */
PG_FUNCTION_INFO_V1(combinable_array_agg_deserialize);
Datum
combinable_array_agg_deserialize(PG_FUNCTION_ARGS)
{
	ArrayType *vals;
...
bucket_agg_state_deserialize(PG_FUNCTION_ARGS)
{
	...
	data_offset = VARDATA(bytes) - (char *)bytes;

	Assert(data_offset < (size_t) nbytes);

	appendBinaryStringInfo(&buf, VARDATA(bytes), nbytes - data_offset);
...
bucket_agg_trans_internal(FunctionCallInfo fcinfo, uint32 ts)
{
	...
	else
	{
		...
		hash = DatumGetUInt32(FunctionCall1Coll(&bas->typ->hash_proc_finfo,
												fcinfo->fncollation, elem));
	}
 ...
 */
PG_FUNCTION_INFO_V1(combinable_string_agg_serialize);
Datum
combinable_string_agg_serialize(PG_FUNCTION_ARGS)
{
 ...
 * external process
 */
PG_FUNCTION_INFO_V1(combinable_string_agg_deserialize);
Datum
combinable_string_agg_deserialize(PG_FUNCTION_ARGS)
{
