...
col_has_index(RangeVar *rv, ColumnRef *col)
{
	...
	Bitmapset *indexed = RelationGetIndexAttrBitmap(rel,
...
get_truncation_from_interval(SelectStmt *select, Node *time, double sf, Node *interval)
{
	...
	step_const->val.sval = *makeString(step_str);
	...
	if (hint)
		ereport(ERROR,
				...
				errmsg(strlen(err) ? "%s" : "sliding window expressions must look like <timestamp column> > clock_timestamp() - <interval>%s", err),
				 aexpr ? parser_errposition(context->pstate, aexpr->location) :
				 0));

	ereport(ERROR,
			(errcode(ERRCODE_SYNTAX_ERROR),
			errmsg(strlen(err) ? "%s" : "sliding window expressions must look like <timestamp column> > clock_timestamp() - <interval>%s", err),
			aexpr ? parser_errposition(context->pstate, aexpr->location) : 0));
}
...
rewrite_from_clause(Node *from)
{
	...
	else if (IsA(from, RangeFunction))
	{
		 ...
		 cvrel = table_openrv(rv, AccessShareLock);
		 table_close(cvrel, AccessShareLock);
...
collect_agg_funcs(Node *node, ContAnalyzeContext *context)
{
	...
	if (IsA(node, FuncCall))
	{
		...
		if (!is_agg)
		{
			clist = FuncnameGetCandidates(func->funcname,
										  list_length(func->args), NIL, true,
										  false, false, true);
			while (clist)
			{
...
get_cont_query_select_stmt(RangeVar *rv, double *step_factor)
{
	...
	sql = pg_get_querydef(query, true);
...
GetContViewQuery(RangeVar *rv)
{
	...
	return parse_analyze_fixedparams(raw, "SELECT", NULL, 0, NULL);
}
...
ValidateParsedContQuery(RangeVar *name, Node *node, const char *sql)
{
	...
	parse_analyze_fixedparams(raw, sql, NULL, 0, NULL);
...
ValidateSubselect(Node *subquery, char *objdesc)
{
	...
	raw = makeNode(RawStmt);
	raw->stmt = (Node *) stmt;
	q = parse_analyze_fixedparams(raw, "subquery", NULL, 0, NULL);
...
TransformSelectStmtForContProcess(RangeVar *mat_relation, SelectStmt *stmt, SelectStmt **viewptr, double sw_step_factor, ContQueryProcType proc_type)
{
	...
	for (i = 0; i < tl_len; i++)
	{
		 ...
		 */
		foreach(clc, context->funcs)
		{
			FuncCall *fcall = (FuncCall *) lfirst(clc);
...
get_agg_name(Oid aggfnoid)
{
	...
	result->val.sval.type = T_String;
	result->val.sval.sval = NameListToString(list_make2(makeString(nsp), makeString(NameStr(proc->proname))));
...
make_finalize_call(Aggref *agg, ColumnRef *cref)
{
	...
	if (transtype == INTERNALOID && OidIsValid(deserfn))
	{
		...
		type_arg->typeName = typeStringToTypeName(get_type_name(type), NULL);
		...
		finalize = makeFuncCall(list_make2(makeString(PIPELINEDB_EXTENSION_NAME),
										   makeString("finalize")), finalizeargs,
	...
	for (i=1; i<finalform->pronargs; i++)
	{
		...
		tc = makeNode(TypeCast);
		tc->typeName = typeStringToTypeName(typname, NULL);
		tc->arg = (Node *) c;
...
GetContQueryAction(ViewStmt *stmt)
{
	...
	foreach(lc, stmt->options)
	{
		...
		if (pg_strcasecmp(def->defname, OPTION_ACTION) == 0)
		{
			...
			Assert(v);
			if (pg_strcasecmp(v, ACTION_TRANSFORM) == 0)
				return TRANSFORM;
			else if (pg_strcasecmp(v, ACTION_MATERIALIZE) == 0)
				return MATERIALIZE;
			else if (pg_strcasecmp(v, ACTION_DUMPED) == 0)
				return DUMPED;
			else
				elog(ERROR, "invalid action \"%s\"", v);
		}
	}
...
GetContWorkerQuery(ContQuery *view)
{
	...
	Query *query = linitial(pg_analyze_and_rewrite_fixedparams(stmt, view->sql,
															   NULL, 0, NULL));
...
is_combine_aggref(Aggref *agg, bool *sw)
{
	...
	Datum d = SysCacheGetAttr(AGGFNOID, tup, Anum_pg_aggregate_aggminitval,
							  &isnull);
	char *magic = NULL;
...
get_combine_aggfnoid(Oid aggfnoid, Oid *rettype)
{
	...
	*rettype = InvalidOid;
	...
	if (!OidIsValid(result))
	{
		 ...
		 */
		HeapTuple ctup = PipelineCatalogLookup(PIPELINECOMBINEAGGFN, 1,
		...
		pc = (Form_pipeline_combine) GETSTRUCT(ctup);
