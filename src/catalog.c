...
PipelineCatalogLookup(int id, int nkeys, ...)
{
	...
	keys = palloc(nkeys * sizeof(Datum));
	...
	/*
	 * If we are here, the entry must either not have been cached, or have
	 * been invalidated. In either case we may need to update the tuple
	 * descriptor.
	 */
	old = MemoryContextSwitchTo(Catalog->context);
	FreeTupleDesc(catalogdesc[id].desc);
	catalogdesc[id].desc = CreateTupleDescCopyConstr(RelationGetDescr(rel));
	MemoryContextSwitchTo(old);
...
PipelineCatalogGetAttr(int id, HeapTuple tup, AttrNumber attr, bool *isnull)
{
	...
	Assert(attr <= catalogdesc[id].desc->natts);
