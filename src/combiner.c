...
get_cached_groups_plan(ContQueryCombinerState *state, List *values)
{
	...
	qlist = pg_analyze_and_rewrite_fixedparams(raw, state->base.query->matrel->relname, NULL, 0, NULL);
	...
	ps->p_sourcetext = state->base.query->sql;

	/*
	 * Index scans do not play well with partitioned matrels, because:
	 *
	 * - for partitioned matrels the optimizer creates an Append node, with
	 *   subplans to access the underlying partition(s);
	 *
	 * - the Append node for IndexScan subplans adds a sort column to the Append
	 *   node's targetlist (see checks around 'pathkeys' in
	 *   create_append_plan()). Which seems to be a bug in PostgreSQL's
	 *   optimizer, because the comment in prepare_sort_from_pathkeys() says it
	 *   is necessary for Sort, MergeAppend and Gather Merge nodes, but not
	 *   plain Append ones;
	 *
	 * - the new sort column is NOT added to subplan's targetlists (i.e.
	 *   IndexScan), contrary to what the comment in create_append_plan() says.
	 *   The target list is not adjusted, because the corresponding call to
	 *   prepare_sort_from_pathkeys() is made with adjust_tlist_in_place ==
	 *   false. Which seems to be another bug in PostgreSQL's optimizer;
	 *
	 * - later on in the query planning, ExecConditionalAssignProjectionInfo()
	 *   detects a mismatch between the Append node's targetlist and the tuple
	 *   descriptor from the subplan (i.e. IndexScan), so it assigns a
	 *   projection;
	 *
	 * - which by itself would not a big issue for regular queries. The only
	 *   difference is that they are executed a little less efficiently due to
	 *   extra work introduced by projections;
	 *
	 * - however, projections break PipelineDB's physical group lookups, because
	 *   the former are only capable of producing virtual tuples, but physical
	 *   group lookups require physical updateable tuples.
	 *
	 * As a result, we disable indexscans for partitioned matrels for the lack
	 * of a better workaround. What it means in practice (and confirmed by
 ...
 * timestamp_round
 *
 * Round a timestamp/timestamptz value to an arbitrary interval
 */
static Datum
timestamp_round(MatRelPartitionKey *key, Interval *i)
{
	TimestampTz timestamp = DatumGetTimestampTz(key->value);
...
get_partition_lower_bound(MatRelPartitionKey *ts, Interval *i)
{
...
get_matrel_partition_for_key(Relation matrel, Datum *values)
{
	...
	int bound_offset;
	int part_oid = InvalidOid;
	bool equal = false;
	bool range_partkey_has_null = false;

	key = RelationGetPartitionKey(matrel);
	partdesc = RelationGetPartitionDesc(matrel, true);
	...
	if (!range_partkey_has_null)
	{
		...
		if (part_index >= 0)
			part_oid = partdesc->oids[part_index];
	}
 ...
 */
static Oid
create_matrel_partition(ContQueryCombinerState *state,
						MatRelPartitionKey *lower_bound)
{
	...
	matrel = table_open(state->base.query->matrelid, AccessShareLock);

	values[0] = TimestampTzGetDatum(lower_bound->value);

	part_oid = get_matrel_partition_for_key(matrel, values);
 ...
 */
static Oid
get_matrel_partition_for_tuple(ContQueryCombinerState *state, Relation matrel,
							   TupleTableSlot *slot, MatRelPartitionKey *ts)
{
	int part_oid = InvalidOid;
	PartitionKey key;
	PartitionDesc partdesc;
	...
	/*
	 * Currently we only support partitioning by a single timestamp or
	 * timestamptz column
	 */
	Assert(key->partnatts == 1);
	Assert(key->parttypid[0] == TIMESTAMPOID || key->parttypid[0] == TIMESTAMPTZOID);
	...
	if (ts)
	{
		ts->value = DatumGetTimestampTz(values[0]);
		ts->typid = get_partition_col_typid(key, 0);
	}
...
close_matrel_partitions(ContQueryCombinerState *state)
{
	...
	while ((entry = (MatRelPartitionEntry *) hash_seq_search(&iter)))
	{
		...
		table_close(entry->rel, RowExclusiveLock);
	}
...
open_target_matrel_partition(ContQueryCombinerState *state, Relation matrel, TupleTableSlot *slot)
{
	...
	part_oid = get_matrel_partition_for_tuple(state, matrel, slot, NULL);
	...
	if (!state->partitions)
	{
		...
		ctl.hash = oid_hash;
		state->partitions = hash_create("MatRelPartitions", 8, &ctl, HASH_ELEM | HASH_FUNCTION);
	}

	Assert(state->partitions);
	entry = (MatRelPartitionEntry *) hash_search(state->partitions, &part_oid, HASH_ENTER, &found);

	if (!found)
	{
		entry->rel = table_open(part_oid, RowExclusiveLock);
...
static void
open_matrel_and_parts(ContQueryCombinerState *state, Relation *pmatrel,
					  List **pparts, LOCKMODE mode)
{
	*pmatrel = table_open(state->base.query->matrelid, mode);

	*pparts = NIL;

	if (IS_PARTITIONED_MATREL(state))
	{
		PartitionDescData *pd = RelationGetPartitionDesc(*pmatrel, true);
		int i;

		for (i = 0; i < pd->nparts; i++)
		{
			Relation rel = table_open(pd->oids[i], mode);

			*pparts = lappend(*pparts, rel);
		}
	}
}

static void
close_matrel_and_parts(Relation matrel, List *parts, LOCKMODE mode)
{
	ListCell *lc;
...
select_existing_groups(ContQueryCombinerState *state)
{
	...
	open_matrel_and_parts(state, &matrel, &parts, RowShareLock);
	...
	close_matrel_and_parts(matrel, parts, NoLock);
...
finish:
	...
	while ((entry = ScanTupleHashTable(existing, &status)) != NULL)
	{
		 ...
		 */
		ExecForceStoreHeapTuple(pt->tuple, slot, false);
...
sync_sw_matrel_groups(ContQueryCombinerState *state, Relation matrel)
{
	...
	ScanKeyData skey[1];
	TimestampTz oldest;
	LOCAL_FCINFO(hashfcinfo, FUNC_MAX_ARGS);
	FmgrInfo flinfo;
	...
	slot = MakeSingleTupleTableSlot(state->slot->tts_tupleDescriptor,
									&TTSOpsMinimalTuple);
	...
	while ((tup = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		...
		if (state->hashfunc)
			hash = slot_hash_group_skip_attr(slot, state->base.query->sw_attno, state->hashfunc, hashfcinfo);
		else
			hash = cv_name_hash;
		...
		entry = LookupTupleHashEntry(state->sw->step_groups, slot, &isnew, NULL);
...
add_cached_sw_tuples_to_overlay_input(ContQueryCombinerState *state)
{
	...
	while ((matrel_entry = ScanTupleHashTable(state->sw->step_groups, &seq)) != NULL)
	{
		...
		d = slot_getattr(slot, state->sw->arrival_ts_attr, &isnull);
...
gc_cached_matrel_tuples(ContQueryCombinerState *state, List *expired)
{
	...
	foreach(lc, expired)
	{
		...
		ExecForceStoreHeapTuple(tup, slot, false);
#ifdef USE_ASSERT_CHECKING
		removed = tuplehash_remove(state->sw->step_groups, slot);
#else
		tuplehash_remove(state->sw->step_groups, slot);
#endif
		...
		TimestampTz this_tick, ResultRelInfo *osri)
{
	...
	while ((entry = (TupleHashEntry) ScanTupleHashTable(state->sw->overlay_groups, &seq)) != NULL)
	{
		...
		ExecStoreHeapTuple(os_tup, slot, false);
		ExecStreamInsert(NULL, osri, slot, NULL);
	}
	...
	foreach(lc, to_delete)
	{
		...
		ExecForceStoreHeapTuple(tup, slot, false);
#ifdef USE_ASSERT_CHECKING
		removed = tuplehash_remove(state->sw->overlay_groups, slot);
#else
		tuplehash_remove(state->sw->overlay_groups, slot);
#endif
...
init_sw_state(ContQueryCombinerState *state, Relation matrel)
{
	...
	state->sw->step_groups = CompatBuildTupleHashTable(state->desc, state->ngroupatts,
			state->groupatts, state->eq_funcs, state->hash_funcs, state->collations, 1000,
	...
	SetTuplestoreDestReceiverParams(state->sw->overlay_dest,
									state->sw->overlay_output,
									state->sw->context, true, NULL, NULL);
	...
	state->sw->overlay_groups = CompatBuildTupleHashTable(state->overlay_desc, n_group_attr,
			group_idx, eq_funcs, hash_funcs, state->collations, 1000, sizeof(OverlayTupleEntry), CurrentMemoryContext, tmp_cxt, false);
}
...
combine(ContQueryCombinerState *state, bool lookup)
{
	...
	open_matrel_and_parts(state, &matrel, &parts, AccessShareLock);
	...
	SetTuplestoreDestReceiverParams(dest, state->combined, state->combine_cxt,
									false, NULL, NULL);
...
sync_combine(ContQueryCombinerState *state)
{
	...
	hslot = MakeSingleTupleTableSlot(slot->tts_tupleDescriptor,
									 &TTSOpsHeapTuple);
	...
	foreach_tuple(slot, state->combined)
	{
		...
		{
			ExecForceStoreHeapTuple(update->tuple, state->prev_slot, false);
			...
			ExecStoreHeapTuple(tup, hslot, false);

			/*
			 * If this is a partitioned matrel, get the partition that this write belongs to
			 */
			if (IS_PARTITIONED_MATREL(state))
				leaf_ri = open_target_matrel_partition(state, ri->ri_RelationDesc, hslot);

			if (!ExecCQMatRelUpdate(leaf_ri, hslot, estate))
				continue;
			...
			ntups_updated++;
			nbytes_updated += HEAPTUPLESIZE + ExecFetchSlotHeapTuple(hslot, true, &should_free)->t_len;
		}
		else
		{
			...
			if (!ExecCQMatRelInsert(leaf_ri, slot, estate))
				continue;
				...
				(os_nulls[OLD_TUPLE] == false || os_nulls[NEW_TUPLE] == false))
		{
			...
			if (e != NULL)
			{
				...
				ExecForceStoreHeapTuple(os_tup, state->os_slot, false);
				ExecStreamInsert(NULL, osri, state->os_slot, NULL);
			}
			else
			{
...
sync_all(ContExecutor *cont_exec)
{
	...
	id = -1;
	while ((id = bms_next_member(tmp, id)) >= 0)
	{
		...
		/*
		 * Set acks and scan_partitions to NIL here so we don't try to access
		 * them for future ticks that happen in a different transaction.
		 */
		state->acks = NIL;
		state->scan_partitions = NIL;

		MemSet(state->group_hashes, 0, state->group_hashes_len);
		MemoryContextReset(state->combine_cxt);
...
assign_output_stream_projection(ContQueryCombinerState *state)
{
	...
	rel = table_open(state->base.query->relid, AccessShareLock);
 ...
 * init_collations
 *
 * Initialize collations for the hash lookup key fields in 'state'.
 */
static void
init_collations(ContQueryCombinerState *state)
{
	state->collations = (Oid *) palloc0(state->ngroupatts * sizeof(Oid));

	for (int i = 0; i < state->ngroupatts; i++)
	{
		AttrNumber			attno = state->groupatts[i];
		Form_pg_attribute	attr  = TupleDescAttr(state->desc, attno - 1);

		state->collations[i] = attr->attcollation;
	}
}
...
init_query_state(ContExecutor *cont_exec, ContQueryState *base)
{
	...
	if (IsA(state->combine_plan->planTree, Agg))
	{
		...
		state->groupatts = palloc0(sizeof(AttrNumber) * state->ngroupatts);

		if (IsA(agg->plan.lefttree, Append))
		{
			Append *append = (Append *) agg->plan.lefttree;
			Plan *subplan = (Plan *) linitial(append->appendplans);
			targetlist = subplan->targetlist;
		}
		else
		{
			targetlist = agg->plan.lefttree->targetlist;
		}
		...
		for (i = 0; i < state->ngroupatts; i++)
		{
			...
			var = (Var *) expr;
			state->groupatts[i] = var->varattno;
		}

		state->groupops = agg->grpOperators;
		state->isagg = true;

		init_collations(state);
		...
		if (state->ngroupatts)
		{
			...
			nargs = list_length(state->hashfunc->args);

			state->hash_fcinfo = palloc0(SizeForFunctionCallInfo(nargs));
			...
			state->hash_fcinfo->fncollation = state->hashfunc->funccollid ?
				state->hashfunc->funccollid : DEFAULT_COLLATION_OID;
			state->hash_fcinfo->nargs = nargs;
 ...
 */
static List *
append_unique_lower_bound(List *l, MatRelPartitionKey *ts)
{
	ListCell *lc;
	List *result = l;
	...
	foreach(lc, l)
	{
		MatRelPartitionKey *lts = (MatRelPartitionKey *) lfirst(lc);

		if (DatumGetBool(DirectFunctionCall2(timestamp_eq,
											 DatumGetTimestamp(lts->value),
											 DatumGetTimestamp(ts->value))))
				return result;
	}
...
read_batch(ContExecutor *exec, ContQueryCombinerState *state, Oid query_id)
{
	...
	while ((itup = ipc_tuple_reader_next(query_id)) != NULL)
	{
		...
		ExecForceStoreHeapTuple(heap_copytuple(itup->tup), state->slot, false);
		...
		if (IS_PARTITIONED_MATREL(state))
		{
			 ...
			 */
			part_oid = get_matrel_partition_for_tuple(state, matrel, state->slot, &ts);
			if (!OidIsValid(part_oid))
			{
				MatRelPartitionKey *lower_bound =
					palloc0(sizeof(MatRelPartitionKey));
				/*
			...
			{
				part_oids = lappend_oid(part_oids, part_oid);
			}
		}
	...
	if (partitions)
	{
		...
		{
			MatRelPartitionKey *lb = (MatRelPartitionKey *) lfirst(lc);
			Oid part_oid;

			part_oid = create_matrel_partition(state, lb);
			part_oids = lappend_oid(part_oids, part_oid);
		}
	...
	if (IS_PARTITIONED_MATREL(state))
	{
		...
		{
			part = lfirst_oid(lc);
			state->scan_partitions = list_append_unique_oid(state->scan_partitions, part);
		}
...
get_min_tick_ms(void)
{
	...
	id = -1;
	while ((id = bms_next_member(queries, id)) >= 0)
	{
	...
	for (;;)
	{
		...
		while ((query_id = ContExecutorStartNextQuery(cont_exec, min_tick_ms)) != InvalidOid)
		{
			...
			PG_TRY();
			{
				...
				MemoryContextReset(state->base.tmp_cxt);
...
GetCombinerLookupPlan(ContQuery *view)
{
	...
	if (state->isagg && state->ngroupatts > 0)
	{
		...
		existing = CompatBuildTupleHashTable(state->desc, state->ngroupatts,
											 state->groupatts, eq_funcs,
											 hash_funcs, state->collations,
											 1000, sizeof(PhysicalTupleData),
		...
		if (rel)
		{
			...
			/* Temporary disable initialization of partitioned queries */
			if (!AttributeNumberIsValid(view->partition_attno))
			{
...
combine_table(PG_FUNCTION_ARGS)
{
	...
	TableScanDesc scan;
	HeapTuple tup;
	FunctionCallInfo hashfcinfo;
	TupleTableSlot *slot;

	hashfcinfo = palloc0(SizeForFunctionCallInfo(FUNC_MAX_ARGS));
	...
	while ((tup = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		ExecStoreHeapTuple(heap_copytuple(tup), slot, false);
		tuplestore_puttupleslot(state->batch, slot);

		if (state->hashfunc)
		{
			uint64 hash =  slot_hash_group(slot, state->hashfunc, hashfcinfo);
		...
		MemoryContextReset(base->tmp_cxt);
