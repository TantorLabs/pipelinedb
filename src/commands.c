 ...
 */
void
PipelineProcessUtility(PlannedStmt *pstmt, const char *sql, bool readOnlyTree,
					   ...
					   DestReceiver *dest, QueryCompletion *qc)
{
	...
	PG_TRY();
	{
		...
		else if (IsA(parsetree, AlterTableStmt))
		{
			...
			else if (stmt->objtype == OBJECT_VIEW && !IsBinaryUpgrade &&
					 RangeVarIsContView(stmt->relation))
			{
				/*
