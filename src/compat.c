...
/*
 * CompatSearchSysCacheLocked
 */
HeapTuple
CompatSearchSysCacheLocked1(int cacheId, Datum key1)
{
#if ((PG_VERSION_NUM >= 150009 && PG_VERSION_NUM < 160000) \
	 || PG_VERSION_NUM >= 160005)
	return SearchSysCacheLocked1(cacheId, key1);
#else
...
/*
 * CompatUnlockTupleInplaceUpdate
 */
void
CompatUnlockTupleInplaceUpdate(Relation relation, ItemPointer tid)
{
#if ((PG_VERSION_NUM >= 150009 && PG_VERSION_NUM < 160000) \
	 || PG_VERSION_NUM >= 160005)
	Assert(IsInplaceUpdateRelation(relation));

	UnlockTuple(relation, tid, InplaceUpdateTupleLock);
#endif
}
