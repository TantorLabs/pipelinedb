...
static shmem_request_hook_type save_shmem_request_hook;
...
char *pipeline_version_str = PIPELINE_VERSION_STR;
char *pipeline_revision_str = PIPELINE_REVISION_STR;
...
pipeline_shmem_startup(void)
{
	...
	PipelineStatsShmemInit();
	ContQuerySchedulerShmemInit();
	MicrobatchAckShmemInit();
}

/*
 * pipeline_shmem_request
 */
static void
pipeline_shmem_request(void)
{
	if (save_shmem_request_hook)
		save_shmem_request_hook();

	RequestAddinShmemSpace(ContQuerySchedulerShmemSize());
	RequestAddinShmemSpace(MicrobatchAckShmemSize());
	RequestAddinShmemSpace(PipelineStatsShmemSize());

	PipelineStatsRequestLWLocks();
}
...
#define BACKTRACE_SEGFAULTS
	...
	DefineCustomIntVariable("pipelinedb.index_fillfactor",
			gettext_noop("Sets the default index fillfactor to use for continuous views."),
			NULL,
			&continuous_view_index_fillfactor,
			50, 1, 100,
			PGC_POSTMASTER, 0,
			NULL, NULL, NULL);
	...
	save_shmem_request_hook = shmem_request_hook;
	shmem_request_hook = pipeline_shmem_request;
