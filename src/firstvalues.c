...
first_values_serialize(PG_FUNCTION_ARGS)
{
	...
	if (state->array)
		array = DatumGetByteaP(DirectFunctionCall1(combinable_array_agg_serialize,
												   PointerGetDatum(state->array)));
...
first_values_deserialize(PG_FUNCTION_ARGS)
{
	...
	if (state->array != NULL)
	{
		...
		state->array = (ArrayBuildState *)
			DatumGetPointer(combinable_array_agg_deserialize(fcinfo));
...
init_first_values_query_state(PG_FUNCTION_ARGS)
{
	...
	if (num_sort > 1)
	{
		...
		attidx = 0;
		foreach(sort_lc, sortlist)
		{
			SortGroupClause *sortcl = (SortGroupClause *) lfirst(sort_lc);
			TargetEntry *tle = get_sortgroupclause_tle(sortcl, aggref->args);
			SortSupport sortkey = &qstate->sortkey[attidx];
			Oid type = exprType((Node *) tle->expr);

			/* the parser should have made sure of this */
			Assert(OidIsValid(sortcl->sortop));

			TupleDescInitEntry(qstate->tup_desc, attidx + 1, NULL, type, -1, 0);
			...
			qstate->sortop[attidx] = sortcl->sortop;
			sort_types[attidx] = type;
			attidx++;
		}
...
compare_values(FirstValuesQueryState *qstate, Datum d1, bool isnull1, Datum d2, bool isnull2)
{
	...
	for (i = 0; i < natts; i++)
	{
		bool n0;
		bool n1;
		Datum attr0 = slot_getattr(qstate->tup_slot1, i + 1, &n0);
		Datum attr1 = slot_getattr(qstate->tup_slot2, i + 1, &n1);
		SortSupport sortkey = qstate->sortkey + i;

		int result = ApplySortComparator(attr0, n0, attr1, n1, sortkey);
