...
composite_to_json(Datum composite, StringInfo result, bool use_line_feeds)
{
	...
	for (i = 0; i < tupdesc->natts; i++)
	{
		...
		if (isnull)
		{
			...
			json_categorize_type(TupleDescAttr(tupdesc, i)->atttypid, false,
								 &tcategory, &outfuncoid);
 ...
 *
 * This is just a thin wrapper around datum_to_json_internal. If the same type
 * will be printed many times, avoid using this; better to do the
 * json_categorize_type lookups only once.
 */
static void
...
combinable_json_object_agg_transfn(PG_FUNCTION_ARGS)
{
	...
	datum_to_json_internal(arg, false, state->str, state->key_category,
	...
	datum_to_json_internal(arg, PG_ARGISNULL(2), state->str, state->val_category,
						   state->val_output_func, false);
...
json_object_int_sum_transfn(PG_FUNCTION_ARGS)
{
	...
	JsonLexContext lex;
	JsonSemAction *sem;
	char *raw = text_to_cstring(PG_GETARG_TEXT_P(1));
	...
	makeJsonLexContextCstringLen(&lex, raw, strlen(raw), GetDatabaseEncoding(), true);
