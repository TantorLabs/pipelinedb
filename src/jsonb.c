...
typedef enum					/* type categories for datum_to_jsonb_internal */
{
...
/*
 * Determine how we want to render values of a given type in datum_to_jsonb_internal.
 *
 ...
 *
 * This is just a thin wrapper around datum_to_jsonb_internal.  If the same type
 * will be printed many times, avoid using this; better to do the
 * jsonb_categorize_type lookups only once.
 */
...
combinable_jsonb_object_agg_transfn(PG_FUNCTION_ARGS)
{
	 ...
	 * above when calling datum_to_jsonb_internal, so we only need to look for
	 * these things.
	 */
