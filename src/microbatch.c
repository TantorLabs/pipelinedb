...
/*
 * We want tuples in the packed microbatch buffer to be MAXALIGNed.
 * microbatch_add_tuple() ensures tuples are properly aligned when
 * accumulating in them a temporary StringInfo buffer. We just need to make
 * sure the address in the packed buffer where the temporary buffer will be
 * copied is also aligned. The following calculations rely on specific packing
 * format implemented in microbatch_pack() and also on the fact that
 * size(tagged_ref_t) is properly aligned and thus, can be excluded from
 * calculations.
 */
#define PACKED_TUPLES_OFFSET (sizeof(microbatch_type_t) + 2 * sizeof(int))
#define PACKED_TUPLES_PADDING (MAXALIGN(PACKED_TUPLES_OFFSET) - \
							   PACKED_TUPLES_OFFSET)
/*
 * We also want query bitmaps in the packed buffer to be MAXALIGNed.
 */
#define PACKED_QUERIES_PADDING MAXIMUM_ALIGNOF
...
microbatch_new(microbatch_type_t type, Bitmapset *queries, TupleDesc desc)
{
	...
	/* Take into account padding for tuples and queries bitmaps */
	mb->packed_size += PACKED_TUPLES_PADDING + PACKED_QUERIES_PADDING;
...
microbatch_add_tuple(microbatch_t *mb, HeapTuple tup, uint64 hash)
{
	...
	/*
	 * We want each tuple in the buffer to be MAXALIGNed, because microbatch
	 * consumers use HeapTuple pointers pointing directly into StringInfo's
	 * buffer. We assume the buffer itself to be palloc'ed, i.e. properly
	 * aligned. Thus we just need to ensure the offset of each tuple is
	 * MAXALIGNed.
	 */
	padding = MAXALIGN(mb->buf->len) - mb->buf->len;
	aligned_size = padding + tup_size;

	if (aligned_size > MAX_PACKED_SIZE)
	...
	if (aligned_size + mb->packed_size + mb->buf->len >= MAX_PACKED_SIZE)
		return false;

	appendStringInfoSpaces(mb->buf, padding);

	Assert(mb->buf->len == MAXALIGN(mb->buf->len));

	appendBinaryStringInfoNT(mb->buf, (char *) tup, HEAPTUPLESIZE);
	appendBinaryStringInfoNT(mb->buf, (char *) tup->t_data, tup->t_len);
...
microbatch_pack(microbatch_t *mb, int *len)
{
	...
	/*
	 * Ensure the data (and hence, each tuple) is aligned. The space for this
	 * padding was accounted for at buffer allocation by adding
	 * PACKED_TUPLES_PADDING to mb->packed_size.
	 */
	pos += PACKED_TUPLES_PADDING;
	...
	if (mb->type == WorkerTuple)
	{
		...
		/*
		 * Pack queries. We want it to be properly aligned, as the microbatch
		 * consomer will use a pointer pointing directly into the packed
		 * buffer. The space for this padding was accounted for at buffer
		 * allocation by adding PACKED_QUERIES_PADDING to mb->packed_size.
		 */
		pos = (char *)MAXALIGN(pos);
...
microbatch_unpack(char *buf, int len)
{
	...
	for (i = 0; i < mb->ntups; i++)
	{
		...
		/* microbatch_add_tuple() ensure each tuple is aligned */
		pos = (char *) MAXALIGN(pos);
	...
	if (mb->type == WorkerTuple)
	{
		...
		/*
		 * Unpack queries. microbatch_pack() ensures they are properly aligned
