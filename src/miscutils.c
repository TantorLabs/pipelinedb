...
MurmurHash3_128(const void *key, const Size len, const uint64_t seed, void *out)
{
	...
	for(i = 0; i < nblocks; i++)
	{
		uint64_t t_k1 = getblock64(blocks,i*2+0);
		uint64_t t_k2 = getblock64(blocks,i*2+1);

		t_k1 *= c1; t_k1  = rotl64(t_k1,31); t_k1 *= c2; h1 ^= t_k1;

		h1 = rotl64(h1,27); h1 += h2; h1 = h1*5+0x52dce729;

		t_k2 *= c2; t_k2  = rotl64(t_k2,33); t_k2 *= c1; h2 ^= t_k2;
...
equalTupleDescsWeak(TupleDesc tupdesc1, TupleDesc tupdesc2, bool check_names)
{
	...
	for (i = 0; i < tupdesc1->natts; i++)
	{
		...
		if (attr1->attcollation != attr2->attcollation)
			return false;
		/* attstattarget, attacl, attoptions and attfdwoptions are not even present... */
	}
...
pipeline_finalize(PG_FUNCTION_ARGS)
{
	...
	if (!fcinfo->flinfo->fn_extra)
	{
		...
		if (!HeapTupleIsValid(tup))
			elog(ERROR, "no pg_proc row found for function \"%s\"",
		...
		deserinfo = palloc0(SizeForFunctionCallInfo(2));
		...
		finalinfo = palloc0(SizeForFunctionCallInfo(1));
...
pipeline_set_ttl(PG_FUNCTION_ARGS)
{
	...
	Relation matrel;
	int ttl = -1;
	char *ttl_colname = ttl_col ? text_to_cstring(ttl_col) : NULL;
	...
	if (ttl_colname)
	{
		matrel = table_openrv(cv->matrel, AccessShareLock);
