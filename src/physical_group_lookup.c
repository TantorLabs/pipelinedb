	...
	if (IsA(outer, NestLoop))
	{
		...
		if (IsA(innerPlan(outer), Append))
		{
			...
			foreach(lc, a->appendplans)
			{
				...
				Oid relid = exec_rt_fetch(s->scanrelid, estate)->relid;
...
output_physical_tuple(TupleTableSlot *slot)
{
	...
	entry = LookupTupleHashEntry(lookup_result, mslot, &isnew, NULL);
...
lnext:
	...
	if (!TTS_IS_BUFFERTUPLE(inner))
	{
		end_lookup_scan(node);
		elog(ERROR, "unsupported access method for relation: %u",
			 inner->tts_tableOid);
	}
