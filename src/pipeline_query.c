...
int continuous_view_index_fillfactor;
...
get_next_id(Relation rel)
{
	...
	if (num_ids)
	{
		...
		ids = palloc(num_ids * sizeof(Oid));
		counts_per_combiner = palloc0(num_combiners * sizeof(int));
...
add_options_for_dump(char *cvname, Oid streamrelid, Oid viewrelid, Oid matrelid, Oid seqrelid, Oid osrelid,
					 Oid pkindexid, Oid lookupindexid, int ttl,
					 AttrNumber ttl_attno, double step_factor,
					 AttrNumber partition_attno, int partition_duration,
					 List *options)
{
	char *relname;
	...
	if (AttributeNumberIsValid(partition_attno) && partition_duration > 0)
	{
		Interval partition_interval;
		char *partition_str;

		options = set_option(options, OPTION_PARTITION_ATTNO,
							 (Node *) makeInteger(partition_attno));

		MemSet(&partition_interval, 0, sizeof(Interval));
		partition_interval.time = partition_duration * USECS_PER_SEC;

		partition_str = (char *)
			DirectFunctionCall1(interval_out, (Datum) &partition_interval);

		options = set_option(options, OPTION_PARTITION_DURATION,
							 (Node *) makeString(partition_str));
	}
...
GetContQueryDef(Oid defrelid)
{
	...
	rel = table_open(defrelid, AccessShareLock);
...
StorePipelineQueryReloptions(Oid relid, List *options)
{
	...
	classtup = CompatSearchSysCacheLocked1(RELOID, ObjectIdGetDatum(relid));
	...
	CompatUnlockTupleInplaceUpdate(pgclass, &classtup->t_self);
 ...
 * add_fillfactor
 *
 * Return a fillfactor DefElem with the specified fillfactor value
 */
List *
add_fillfactor(List *options, int fillfactor)
{
	DefElem *ff = makeDefElem(OPTION_FILLFACTOR, (Node *) makeInteger(fillfactor), -1);
...
create_lookup_index(RangeVar *cv, Oid matrelid, RangeVar *matrel, SelectStmt *select, bool is_sw)
{
	...
	index->options = add_fillfactor(NIL, continuous_view_index_fillfactor);

	address = DefineIndex(matrelid, index, InvalidOid, InvalidOid, InvalidOid,
...
create_pkey_index(RangeVar *cv, Oid matrelid, RangeVar *matrel, char *colname, char *partition_by)
{
	...
	if (partition_by)
	{
		...
		/*
		 * Assume the partition key column is not explicitly marked NOT NULL,
		 * because constraints are currently unsupported on streams. Postgres
		 * implicitly marks primary key columns NOT NULL when executing
		 * CREATE/ALTER TABLE. Since we are using lower-level APIs that do not
		 * perform that step for us, we need to do it here on our own.
		 */
		cmd = makeNode(AlterTableCmd);
		cmd->subtype = AT_SetNotNull;
		cmd->name = pstrdup(partition_by);
		cmds = lappend(NIL, cmd);

		EventTriggerAlterTableStart((Node *) index);
		AlterTableInternal(matrelid, cmds, true);
...
DefineContTransform(Oid relid, Oid defrelid, Oid streamrelid, Oid typoid, Oid osrelid, List **optionsp, Oid *ptgfnid)
{
	...
	if (GetOptionAsString(options, OPTION_TGFN, &funcname))
	{
		...
		{
			Form_pg_proc form;
			HeapTuple trgtup = SearchSysCache1(PROCOID,
											   ObjectIdGetDatum(tgfnoid));
			List *qualified = NIL;
			DefElem *opt;

			form = (Form_pg_proc) GETSTRUCT(trgtup);
...
create_cv_row_from_dump(Relation pq, RangeVar *name, Query *query, List *options)
{
	...
	char *relname;
	Oid pqid = InvalidOid;
	int partition_attno = InvalidAttrNumber;
	 ...
	 */
	if (GetOptionAsInteger(options, OPTION_PARTITION_ATTNO, &partition_attno))
	{
		char *partition_duration_str;
		Interval *pbi;

		if (!GetOptionAsString(options, OPTION_PARTITION_DURATION,
							   &partition_duration_str))
		{
			elog(ERROR, "partition_attno specified without partition_duration");
		}

		pbi = (Interval *) DirectFunctionCall3(interval_in,
											   (Datum) partition_duration_str, 0,
											   (Datum) -1);
		partition_duration = IntervalToEpoch(pbi);
	...
	DefineContView(pq, overlayrelid, streamrelid, matrelid, seqrelid, ttl,
				   ttl_attno, sf, partition_duration, partition_attno, &pqid);
	CommandCounterIncrement();
...
record_ct_dependencies(Oid relid, Oid osrelid, Oid fnoid, Oid defrelid, SelectStmt *stmt, Query *query, List *args)
{
	...
	if (fnoid == GetInsertIntoStreamOid())
	{
		...
		foreach(lc, args)
		{
			...
			RangeVar *rv =
				makeRangeVarFromNameList(stringToQualifiedNameList(strVal(v),
																   NULL));
			Relation srel;
			TupleDesc desc;
...
SyncContView(RangeVar *name)
{
	...
	pq = OpenPipelineQuery(RowExclusiveLock);
	ReleaseSysCache(tup);
	...
	ClosePipelineQuery(pq, RowExclusiveLock);
...
ExecCreateContViewStmt(RangeVar *view, Node *sel, List *options, const char *querystring)
{
	...
	cont_query = parse_analyze_fixedparams(raw, querystring, NULL, 0, NULL);
	...
	cont_select_sql = pg_get_querydef(cont_query, true);
	...
	query = parse_analyze_fixedparams(raw, cont_select_sql, NULL, 0, NULL);
	...
	if (partition_by && partition_duration > 0)
	{
		...
		ps->strategy = PARTITION_STRATEGY_RANGE;
	...
	options = add_options_for_dump(view->relname, streamrelid, overlayid, matrelid, seqrelid,
								   osrelid, pkey_idx_oid, lookup_idx_oid, ttl,
								   ttl_attno, sf, partition_attno,
								   partition_duration, options);
...
GetContQueryForId(Oid id)
{
	...
	cq->ttl = row->ttl;
	cq->partition_attno = DatumGetInt16(PipelineCatalogGetAttr(PIPELINEQUERYID, tup, Anum_pipeline_query_part_attno, &isnull));

	if (AttributeNumberIsValid(cq->partition_attno))
	{
		cq->partition_duration = palloc0(sizeof(Interval));
		cq->partition_duration->time = DatumGetInt32(PipelineCatalogGetAttr(PIPELINEQUERYID, tup, Anum_pipeline_query_part_duration, &isnull)) * USECS_PER_SEC;
	}
	...
	cq->sql = pg_get_querydef(copyObject(query), true);
	cq->cvdef = query;
...
ExecCreateContTransformStmt(RangeVar *transform, Node *stmt, List *options, const char *querystring)
{
	...
	query = parse_analyze_fixedparams(raw, querystring, NULL, 0, NULL);
	ValidateContQuery(query);
...
SyncContQueryDefRel(Oid cqrelid)
{
	...
	tup = CompatSearchSysCacheLocked1(RELOID, ObjectIdGetDatum(cq->defrelid));
	...
	CompatUnlockTupleInplaceUpdate(pgclass, &tup->t_self);
...
SyncStreamReaderDefRels(Oid streamrelid)
{
	...
	pipeline_query = table_open(PipelineQueryRelationOid, AccessShareLock);
