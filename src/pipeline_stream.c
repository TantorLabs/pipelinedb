...
SyncPipelineStreamReaders(void)
{
	...
	table_close(pipeline_stream, RowExclusiveLock);
	...
	if (stream_targets && strlen(stream_targets) && readers)
	{
		...
		foreach(lc, streams)
		{
			...
			while (++i)
			{
				...
				Datum quals_datum =
					DirectFunctionCall3Coll(split_part, DEFAULT_COLLATION_OID,
											view_datum, qual_split,
											Int32GetDatum(i));
...
SyncPipelineStream(void)
{
	...
	pipeline_stream = OpenPipelineStream(RowExclusiveLock);
