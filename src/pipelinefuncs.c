...
#define PIPELINEDB_VERSION_TEMPLATE "Tantor PipelineDB %s at revision %s"
...
pipeline_get_streams(PG_FUNCTION_ARGS)
{
	...
	while ((tup = heap_getnext(data->scan, ForwardScanDirection)) != NULL)
	{
		...
		else
		{
			...
			bytes = (bytea *) PG_DETOAST_DATUM(tmp);
			...
			while ((j = bms_next_member(bms, j)) >= 0)
			{
				...
				view_rel = try_relation_open(view->relid, AccessShareLock);
				if (!view_rel)
					continue;
				...
				if (!RelationIsVisible(view->relid))
					cq_name =
						quote_qualified_identifier(
							get_namespace_name(RelationGetNamespace(view_rel)),
			...
			values[2] = PointerGetDatum(construct_array_builtin(qdatums,
																nqueries,
																TEXTOID));
		}
...
pipeline_get_views(PG_FUNCTION_ARGS)
{
	...
	while ((tup = heap_getnext(data->scan, ForwardScanDirection)) != NULL)
	{
		...
		values[4] = CStringGetTextDatum(pg_get_querydef(query, true));
...
pipeline_get_transforms(PG_FUNCTION_ARGS)
{
	...
	while ((tup = heap_getnext(data->scan, ForwardScanDirection)) != NULL)
	{
		...
		query = GetContQueryDef(row->defrelid);
		values[6] = CStringGetTextDatum(pg_get_querydef(query, true));
...
pipeline_get_worker_querydef(PG_FUNCTION_ARGS)
{
	...
	if (!RangeVarIsContView(rv) && !RangeVarIsContTransform(rv))
		elog(ERROR, "\"%s\" is not a continuous view or transform",
...
pipeline_get_combiner_querydef(PG_FUNCTION_ARGS)
{
	...
	if (!RangeVarIsContView(rv))
		elog(ERROR, "%s is not a continuous view",
			 text_to_cstring(name));
