				   ...
				   bool rewrite_combines)
{
	...
	QuerySetContQueryId(view->cvdef, view->id);
...
get_worker_plan(ContQuery *view)
{
	...
	plan = get_plan_from_stmt(view, stmt, false, true);
...
add_tuplestore_scan_path(PlannerInfo *root, RelOptInfo *rel, Index rti, RangeTblEntry *rte)
{
	...
	if (set_rel_pathlist_hook != add_tuplestore_scan_path)
	{
		/*
		 * The is an ugly workaround for Columnar's enable_custom_scan feature.
 ...
 */
static PlannedStmt *
get_tuplestore_scan_plan(ContQuery *view, RawStmt *node, bool is_combine, bool rewrite_combines)
{
	PlannedStmt *result;
	...
	PG_TRY();
	{
		...
		result = get_plan_from_stmt(view, node, is_combine, rewrite_combines);
...
GetContViewOverlayPlan(ContQuery *view)
{
	...
	result = get_tuplestore_scan_plan(view, raw, false, true);
...
get_combiner_plan(ContQuery *view)
{
	...
	combiner_plan = get_tuplestore_scan_plan(view, result, true, true);
...
static List *
get_custom_scans_from_append(Append *node)
{
	ListCell *lc;
	List *result = NIL;
	...
	foreach(lc, node->appendplans)
	{
		...
		else if (IsA(s, Result) && IsA(s->plan.lefttree, CustomScan))
			result = lappend(result, s->plan.lefttree);
	}
...
SetCombinerPlanTuplestorestate(PlannedStmt *plan, Tuplestorestate *tupstore)
{
	...
	if (IsA(plan->planTree, CustomScan))
		scans = lappend(scans, (CustomScan *) plan->planTree);
	else if ((IsA(plan->planTree, Agg)) &&
			IsA(plan->planTree->lefttree, CustomScan))
		scans = lappend(scans, (CustomScan *) plan->planTree->lefttree);
	else if (IsA(plan->planTree, Agg) &&
			IsA(plan->planTree->lefttree, Sort) &&
			IsA(plan->planTree->lefttree->lefttree, CustomScan))
		scans = lappend(scans,
						(CustomScan *) plan->planTree->lefttree->lefttree);
	else if (IsA(plan->planTree, Agg) &&
			IsA(plan->planTree->lefttree, Result) &&
			IsA(plan->planTree->lefttree->lefttree, CustomScan))
		scans = lappend(scans,
						(CustomScan *) plan->planTree->lefttree->lefttree);
		...
		scans = get_custom_scans_from_append((Append *) plan->planTree);
	else if (IsA(plan->planTree, Agg) &&
			 IsA(plan->planTree->lefttree, Append))
		scans = get_custom_scans_from_append((Append *) plan->planTree->lefttree);
	else if (IsA(plan->planTree, Result) &&
			 IsA(plan->planTree->lefttree, CustomScan))
		scans = lappend(scans, plan->planTree->lefttree);
	...
	foreach(lc, scans)
	{
		CustomScan *scan = (CustomScan *) lfirst(lc);
