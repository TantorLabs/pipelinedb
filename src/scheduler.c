...
debug_segfault(SIGNAL_ARGS)
{
	...
	fprintf(stderr, "Tantor PipelineDB version: %s at revision %s\n", pipeline_version_str, pipeline_revision_str);
...
terminate_database_workers(ContQueryDatabaseMetadata *db_meta)
{
	...
	for (i = 0; i < NUM_BG_WORKERS_PER_DB; i++)
	{
		BackgroundWorkerHandle *handle = db_meta->db_procs[i].bgw_handle;
...
start_database_workers(ContQueryDatabaseMetadata *db_meta)
{
	...
	if (failed > 0)
	{
		...
		elog(ERROR, "failed to start %d pipelinedb workers in database \"%s\"."
			 " consider increasing max_worker_processes by %d",
			 failed, NameStr(db_meta->db_name), failed);
	}
...
reaper(void)
{
	...
	foreach(lc, DatabaseList)
	{
		...
		if (!db_meta->running)
			elog(WARNING,
				 "some background workers for database \"%s\" are not running",
				 NameStr(db_meta->db_name));
	}
}
