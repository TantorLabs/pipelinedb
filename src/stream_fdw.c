...
int stream_insert_level = STREAM_INSERT_SYNCHRONOUS_RECEIVE;
...
BeginStreamScan(ForeignScanState *node, int eflags)
{
	...
	foreach(lc, colnames)
	{
		String *v = (String *) lfirst(lc);
		namestrcpy(&(TupleDescAttr(state->pi->outdesc, i++)->attname), v->sval);
	}
...
exec_stream_project(StreamScanState *node, ipc_tuple *itup)
{
	...
	ExecForceStoreHeapTuple(itup->tup, pi->slot, false);
