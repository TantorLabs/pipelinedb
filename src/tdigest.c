...
TDigestCompress(TDigest *t)
{
	...
	if (t->total_weight > 0)
	{
		...
		num_centroids = args->idx + 1;
		t->max = Max(t->max, args->max);
	}

	if (num_centroids > t->num_centroids)
	{
		Size newsize = sizeof(TDigest) + (sizeof(Centroid) * num_centroids);

		if (MemoryContextContains(CurrentMemoryContext, t))
			t = realloc_tdigest(t, newsize);
		else
		{
			TDigest *new = (TDigest *) palloc(newsize);
			memcpy(new, t, sizeof(TDigest));
			t = new;
		}
	}

	t->num_centroids = num_centroids;
