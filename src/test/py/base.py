...
BOOTSTRAPPED_BASE = b'../tmp/.pdbbase'
INSTALL_FORMAT = b'../tmp/.pdb-%d'
...
class PipelineDB(object):
  def __init__(self, data_dir=None):
    ...
    if os.path.exists(BOOTSTRAPPED_BASE):
      ...
      bootstrap_version = stdout.decode('utf-8').strip().split()[1].split('.')[0]
      match = re.match(r'^(\d+)', bootstrap_version)
      bootstrap_version = int(match.group(1))
      install_version = 0
  ...
  def data_dir(self):
    ...
    # highest index + 1. Install dirs are of the form: ../tmp/.pdb-<n>.
    index = max([int(l.split('-')[1]) for l in os.listdir('../tmp')
