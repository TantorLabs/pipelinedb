...
def test_partitioned_cv(pipeline, clean_db):
  """
  Verify that we can dump and restore partitioned CVs using INSERT statements
  """
  pipeline.create_stream('part_stream', x='int', ts='timestamp')
  q = """
  SELECT ts, x FROM part_stream
  """
  pipeline.create_cv('cv_part_1h', q, partition_by='ts',
                     partition_duration='1 hour')

  pipeline.create_cv('cv_part_2y', q, partition_by='ts',
                     partition_duration='2 years')

  pipeline.insert('part_stream', ('x','ts'),
                  [
                    ...
                    (5, '2026-04-11 14:53:07.82049')
                  ]
                  )

  def _verify():
    result = pipeline.execute("""
    SELECT partition_key_attno FROM pipelinedb.cont_query WHERE partition_duration=3600
    """)[0]
    assert result['partition_key_attno'] == 1

    # 63115200 is the number of seconds in 2 years
    result = pipeline.execute("""
    SELECT partition_key_attno FROM pipelinedb.cont_query WHERE partition_duration=63115200
    """)[0]
    assert result['partition_key_attno'] == 1
    ...
    WHERE  inhparent = 'cv_part_1h_mrel'::regclass""")[0]
    assert result['partitions'] == 5

    result = pipeline.execute("""
    SELECT count(*) AS partitions
    FROM   pg_catalog.pg_inherits
    WHERE  inhparent = 'cv_part_2y_mrel'::regclass""")[0]
    assert result['partitions'] == 2

  _verify()

  _dump(pipeline, 'test_partitioned_cv.sql')

  pipeline.drop_all()

  _restore(pipeline, 'test_partitioned_cv.sql')

  _verify()

  # Now verify that we can successfully add more data to the restored
  # partitioned CV and auto-partitioning works as expected
  pipeline.insert('part_stream', ('x','ts'),
                  [
                    (6, '2028-04-11 13:53:07.82049'),
                    (7, '2024-09-06 20:14:02.25631'),
                  ]
                  )
...
def test_fillfactor(pipeline, clean_db):
  """
  Verify that we can dump and restore CVs with custom fillfactors
  """

  pipeline.stop()
  pipeline.run({'pipelinedb.fillfactor': '10'})

  pipeline.create_stream('s', x='int', ts='timestamp')
  q = """
  SELECT ts, x FROM s
  """
  pipeline.create_cv('cv_ff10', q)

  pipeline.create_cv('cv_part_ff10', q, partition_by='ts',
                     partition_duration='1 hour')

  pipeline.insert('s', ('x','ts'), [(1, '2024-11-12 22:02:01.123')])

  def _verify():
    result = pipeline.execute("""
    SELECT reloptions FROM pg_class WHERE relname='cv_ff10_mrel'
    """)[0]

    assert result['reloptions'] == ['fillfactor=10']

    result = pipeline.execute("""
    SELECT reloptions FROM pg_class WHERE relname='cv_part_ff10_mrel'
    """)[0]

    assert result['reloptions'] is None

    result = pipeline.execute("""
    SELECT c.relname, c.reloptions
    FROM   pg_class c JOIN pg_inherits i ON c.oid = i.inhrelid
    WHERE  inhparent = 'cv_part_ff10_mrel'::regclass""")[0]

    assert result['reloptions'] == ['fillfactor=10']

  _verify()

  _dump(pipeline, 'test_fillfactor.sql')

  pipeline.drop_all()

  pipeline.stop()
  pipeline.run({'pipelinedb.fillfactor': '20'})

  _restore(pipeline, 'test_fillfactor.sql')

  _verify()

  # Verify that new partitions use the updated fillfactor parameter

  pipeline.insert('s', ('x','ts'), [(2, '2024-11-12 23:02:01.123')])

  result = pipeline.execute("""
  SELECT c.relname, c.reloptions
  FROM   pg_class c JOIN pg_inherits i ON c.oid = i.inhrelid
  WHERE  inhparent = 'cv_part_ff10_mrel'::regclass
  ORDER BY c.relname""")[1]

  assert result['reloptions'] == ['fillfactor=20']

def test_index_fillfactor(pipeline, clean_db):
  """
  Verify that we can dump and restore CVs with custom index fillfactors
  """

  pipeline.stop()
  pipeline.run({'pipelinedb.index_fillfactor': '33'})

  pipeline.create_stream('s', x='int', ts='timestamp')
  q = """
  SELECT ts, COUNT(*) FROM s GROUP BY ts
  """
  pipeline.create_cv('cv_ff33', q)

  pipeline.create_cv('cv_part_ff33', q, partition_by='ts',
                     partition_duration='1 hour')
  ...
  def _verify():
    result = pipeline.execute("""
    SELECT reloptions FROM pg_class WHERE relname='cv_ff33_mrel_pkey'
    """)[0]

    assert result['reloptions'] == ['fillfactor=33']

    result = pipeline.execute("""
    SELECT reloptions FROM pg_class WHERE relname='cv_ff33_mrel_expr_idx'
    """)[0]

    assert result['reloptions'] == ['fillfactor=33']

    result = pipeline.execute("""
    SELECT reloptions FROM pg_class WHERE relname='cv_part_ff33_mrel_pkey'
    """)[0]

    assert result['reloptions'] == ['fillfactor=33']

    result = pipeline.execute("""
    SELECT reloptions FROM pg_class WHERE relname='cv_part_ff33_mrel_expr_idx'
    """)[0]
    ...
    SELECT c.relname, c.reloptions
    FROM   pg_class c JOIN pg_inherits i ON c.oid = i.inhrelid
    WHERE  inhparent = 'cv_part_ff33_mrel_pkey'::regclass""")[0]
    ...
    SELECT c.relname, c.reloptions
    FROM   pg_class c JOIN pg_inherits i ON c.oid = i.inhrelid
    WHERE  inhparent = 'cv_part_ff33_mrel_expr_idx'::regclass""")[0]
  ...
  pipeline.stop()
  pipeline.run({'pipelinedb.fillfactor': '42'})

  _restore(pipeline, 'test_fillfactor.sql')

  _verify()

  # Verify that new partitions use the old index fillfactor parameter, i.e. the
  # one that was configured at CV creation time. The reason is that CV indexes
  # on partitions are inherited from the parent matrel rather than created
  # explicitly
  ...
  SELECT c.relname, c.reloptions
  FROM   pg_class c JOIN pg_inherits i ON c.oid = i.inhrelid
  WHERE  inhparent = 'cv_part_ff33_mrel_pkey'::regclass
  ORDER BY c.relname""")[1]
  ...
  SELECT c.relname, c.reloptions
  FROM   pg_class c JOIN pg_inherits i ON c.oid = i.inhrelid
  WHERE  inhparent = 'cv_part_ff33_mrel_expr_idx'::regclass
  ORDER BY c.relname""")[1]
