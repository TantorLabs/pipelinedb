...
CREATE VIEW test_sw_hll_agg0 AS SELECT k::text, regexp_replace(hll_print(hll_agg(x::integer)), ', size = [0-9]+kB', '') AS hll_print FROM test_hll_agg_stream WHERE (arrival_timestamp > clock_timestamp() - interval '1 hour') GROUP BY k;
...
SELECT k, regexp_replace(hll_print(hll_agg), ', size = [0-9]+kB', '') AS hll_print FROM test_hll_agg0 ORDER BY k;
...
SELECT regexp_replace(hll_print(combine(hll_agg)), ', size = [0-9]+kB', '') AS hll_print FROM test_hll_agg0;
