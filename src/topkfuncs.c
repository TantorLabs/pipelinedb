...
topk_print(PG_FUNCTION_ARGS)
{
	...
	initStringInfo(&buf);
	appendStringInfo(&buf, "{ k = %d, m = %d, h = %d, count = %ld, length = %d, size = %ldkB }", fss->k, fss->m, fss->h, (long) fss->count, FSSMonitoredLength(fss), FSSSize(fss) / 1024);

	PG_RETURN_DATUM(CStringGetTextDatum(buf.data));
}
