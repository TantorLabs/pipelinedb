		...
		List *tlist, List *clauses, List *custom_plans)
{
	...
	groupref = palloc0(desc->natts * sizeof(Index));
	...
	if (physical_tlist)
	{
		...
		for (attrno = 1; attrno <= desc->natts; attrno++)
		{
			...
			var = makeVar(rel->relid, attrno, att_tup->atttypid, att_tup->atttypmod, att_tup->attcollation, 0);
...
create_tuplestore_scan_state(struct CustomScan *cscan)
{
	...
	memcpy(&scanstate->store, ptr->sval, sizeof(Tuplestorestate *));
...
tuplestore_next(struct CustomScanState *node)
{
	...
	TupleTableSlot *slot = MakeSingleTupleTableSlot(
		node->ss.ss_ScanTupleSlot->tts_tupleDescriptor, &TTSOpsMinimalTuple);

	if (!tuplestore_gettupleslot(state->store, true, false, slot))
		return NULL;

	/*
	 * Postgres 13+ as of commit 36d22dd95b does not generate EEOP_*_FETCHSOME
	 * operations as an optimizations for slots that are always known to be
	 * virtual. We always return a MinimalTupleSlot from this function, which
	 * is always processed as a virtual slot later (TODO: figure out why),
	 * leading to incorrect usage of 'unfetched' attributes.
	 */
